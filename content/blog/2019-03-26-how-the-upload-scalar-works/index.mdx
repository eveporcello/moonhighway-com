...
The Apollo Server incorporates an package called [graphql-upload](https://github.com/jaydenseric/graphql-upload) (previously `apollo-upload-server`). This package will retrieve files from the `Upload` scalar and pass the file stream in a Promise directly to your resolver:

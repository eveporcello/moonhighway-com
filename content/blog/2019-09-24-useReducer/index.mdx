...
`useReducer` takes in the reducer function and the initial state, `false`. Then we'll set the `onChange` function to `setChecked` which will call the reducer function.
